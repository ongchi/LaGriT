!  Modified heavily for 64 bit version
!  changed malloc to util_malloc wrappers in mmsc.c 
!  which allows better capture of errors and warnings 
!  mm2000.h is compiled from mm2000_header.F for preprocess definitions
!  changed output to allow large numbers (bigger than int) for size reporting
!
...
!!
!PVCS    
!PVCS       Rev 1.10   31 May 2007 14:40:28   gable
!PVCS    Modified error reporting to include partition, array and length.
!PVCS    
!PVCS       Rev 1.9   11 Aug 2005 09:15:32   dcg
!PVCS    change pointer addition to function call that adds
!PVCS    pointers as unsigned integers -- this avoids
!PVCS    problems with platforms where sign bit is part of
!PVCS    the address
!PVCS    
!PVCS       Rev 1.8   21 Mar 2003 15:29:10   gable
!PVCS    Added output at the end of mmprint to print out total
!PVCS    amount of memory associated with memory managed arrays.
!PVCS    
!PVCS       Rev 1.7   11 Mar 2003 07:54:04   gable
!PVCS    Changed format in mmprint to allow larger integers.
!PVCS    
!PVCS       Rev 1.6   14 Nov 2000 09:37:02   dcg
!PVCS    remove warning for block not found
!PVCS
!PVCS       Rev 1.5   12 Oct 2000 13:35:52   dcg
!PVCS    comment out warning from mmrelblk if array does not exist
!PVCS
!PVCS       Rev 1.4   24 Aug 2000 09:09:44   dcg
!PVCS    remove debug print statments
!PVCS
!PVCS       Rev 1.3   23 Aug 2000 14:16:08   dcg
!PVCS    change mmprint to print in increasing address order
!PVCS
!PVCS       Rev 1.0   23 Aug 2000 11:17:58   dcg
!PVCS    Initial revision.
!PVCS
!PVCS       Rev 1.2   Fri Apr 07 08:34:44 2000   dcg
!PVCS    rename blockdata
!PVCS    replace f90 syntax with f77 (specifically f77 does not recognize plain 'do')
!PVCS
!PVCS       Rev 1.1   Thu Apr 06 13:15:40 2000   nnc
!PVCS    Fixed typo in mm_ovall name.
!PVCS
!PVCS       Rev 1.0   Thu Apr 06 10:49:48 2000   nnc
!PVCS    Initial revision.
...
! debug
!     print*
!     print*,'Begin mmgetblk: array, partition, ptr, len, type, status:'
!     print*,'                ',aname(1:icharlnf(aname)),'  ',
!    & pname(1:icharlnf(pname)), aptr,length,type,status 

call mmverify ()
 
call mm_index_by_name (aname, pname, index)
if (index > 0) then  ! Array already exists
  status = -17
...
!     Cannot allocate memory with length less than 1
if (length < 1) then
  if (length<999999 .and. length>-99999) then
  ...
  call mm_error ('MMGETBLK', 'Invalid length request. Array: ' &
        // aname(1:icharlnf(aname)) // '    in partition: ' // &
           pname(1:icharlnf(pname)) // &
        ' with length: ' // char_length(1:icharlnf(char_length)) )
  goto 999
end if
 
if (type < 1 .or. type > 3) then ! Valid types are 1,2,3
  write(char_type, 100) type
  status = -21
  call mm_error ('MMGETBLK', 'Invalid type request. Array: ' &
        // aname(1:icharlnf(aname)) // '    in partition: ' // &
        pname(1:icharlnf(pname)) // &
        '   of type ' // char_type(1:icharlnf(char_type)) )
  goto 999
end if
100 format(i12)
200 format(i6)
...
!     Check that prolog + length*byte size will be a valid number
!     can not try to allocate number larger than max unsigned int
!     malloc will allocate the nonsense number and seg fault will
!     occur later when the unallocated part of memory is used
rbytes = real(PROLOG_BYTES) + real(EPILOG_BYTES)
rbytes = rbytes + (real(BYTES_PER_CHAR)*real(length))

if (rbytes >= RMAX_ALLOCATE ) then
  status = -21
  char_length = '*************'
...
!  debug
!     print*,'mm2000 -- util_malloc 1 called with bytes: ',block_bytes
pptr = util_malloc(block_bytes)
!     print*,'malloc pptr: ',pptr
if ( pptr == 0) then
   status = -1
...
!     Check for errors
idone = 1
if (status /= 0 .and. idone==0) then
  write(*,*)'MMGETBLK: return early with error flag: ',status
end if
if (status /= 0 .and. idone==1) then
...
call mm_index_by_name (aname, pname, index)
if (index == 0) then  ! Array doesn't exist
  status = -16
  call mm_error ('MMNEWLEN', &
         'Array ' // aname // ' in ' // pname // 'does not exist')
end if
 
if (length < 1) then ! Cannot allocate memory with lenght less than 1.
  write(char_length,100)length
  status = -21
  call mm_error ('MMNEWLEN', 'Invalid length request. Array: ' &
        // aname // ' in partition: ' // pname // &
        ' with length: ' // char_length )
end if
100 format(i12)
...
!  debug
!     print*,'mm2000 --- util_realloc: called with bytes: ',block_bytes
...
!    &      'Array ' // aname // ' in ' // pname // ' does not exist')
  ...
  if (aname == ad_name(1,index) .and. &
           pname == ad_name(2,index)) then
...
do while (index > 0)
  ...
  stomped_on = .false.
  do j = 1, AD_SIZE
    if (prolog(j) /= ad_addr(j,index)) stomped_on = .true.
  end do
  if (stomped_on) then
    corrupted = .true.
  ...
  stomped_on = .false.
  do j = 1, AD_SIZE
    if (epilog(j) /= ad_addr(j,index)) stomped_on = .true.
  end do
  if (stomped_on) then
    corrupted = .true.
...
!     Totals over sizeof int give garbage values
!     Use real to report large numbers
isum = 0
itmp = 0
xsum = 0.
do k=1,icount
if    (ad_addr(4,k) == 1) then
   itmp = (ad_addr(5,k)*BYTES_PER_INT)
   xsum = xsum + (ad_addr(5,k)*BYTES_PER_INT)
else if(ad_addr(4,k) == 2) then
   itmp = (ad_addr(5,k)*BYTES_PER_REAL)
   xsum = xsum + (ad_addr(5,k)*BYTES_PER_REAL)
else if(ad_addr(4,k) == 3) then
   itmp = (ad_addr(5,k)*BYTES_PER_CHAR)
   xsum = xsum + (ad_addr(5,k)*BYTES_PER_CHAR)
end if

!     now add values and protect against int size restrictions 
!     WARNING range size of signed long integer is +- 2147483647
if (BYTES_PER_INT < 8 ) then
   if (isum>0.and.((isum+itmp)>2147483000)) then
      isum=-99
   else
      isum=isum+itmp
   end if
...
! avoid erroneous totals by using g or en format
! note - older compilers do not like en format
...
idone = 0
if (next == 0) then ! No free AD; allocate more.
...
do i=1,100000000
  ...
  if (aname == ad_name(1,index) .and. &
           pname == ad_name(2,index)) return
  index = link(index)
end do

return
end subroutine mm_index_by_name
...
do i=1,100000000
  ...
  if (aptr == ad_addr(2,index)) return
  index = link(index)
end do

return
end subroutine mm_index_by_addr
